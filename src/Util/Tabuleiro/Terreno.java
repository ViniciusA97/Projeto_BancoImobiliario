...
public abstract class Terreno implements Comercial, Casa {
	...
	public void pagaAluguel(Jogador recebe, Jogador paga, Observer o, Jogadores j) throws SemSaldoException {// paga
																												// aluguel
																												// da
																												// propriedade
	...
	public void fazAcao(Comandos cmd, Jogadores j) {// faz a ação referente a Terreno
		...
		if (proprietario == null) {
			if (j.getJogadorDaVez().getDinheiro() >= preco) {
				...
				try {
					compra(j, o);
					if (verificaMonopolioPorCor(j.getJogadorDaVez())
							|| verificaPreMonopolioPorCor(j.getJogadorDaVez())) {
